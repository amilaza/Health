...
- (void)setDefaultParameters
{
    ...
    _fillColorOne = [[UIColor lightGrayColor] colorWithAlphaComponent:0.60];
    _fillColorTwo = [[UIColor grayColor] colorWithAlphaComponent:0.60];
    _verticalGridStep = 3;
    ...
    _indexLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:6];
    
    _valueLabelBackgroundColor = [UIColor clearColor];
    _valueLabelTextColor = [UIColor blackColor];
    _valueLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:5];
    _valueLabelPosition = ValueLabelRight;
}

- (void)setChartData:(NSArray *)chartData shouldDisplayDatapoints:(BOOL)points
{
...
- (void)setChartData2:(NSArray *)chartData shouldDisplayDatapoints:(BOOL)points
{
    if (chartData.count == 0) {
        return;
    }
    ...
    CGFloat minBoundSharp = [self minVerticalBound];
    CGFloat maxBoundSharp = [self maxVerticalBound];
//    _data = [NSMutableArray arrayWithArray:chartData];
    CGFloat boundLimit = (maxBoundSharp - minBoundSharp);
    boundLimit = (boundLimit != 0) ? boundLimit : 1;
    CGFloat scale = _axisHeight / boundLimit;
    CGFloat widthCell = self.frame.size.width / (_horizontalGridStep - 1);
    for(int i = 1 ;i<_data.count - 1; i++) {
        CGPoint p = [self getPointForIndex:i withScale:scale];
        p.y +=  _minBound * scale;
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(-widthCell/3 + self.bounds.origin.x + i * widthCell, p.y, 2*widthCell/3, self.bounds.size.height- p.y)];
        view.backgroundColor = BLUE_SIMBLE_LOGO;//[UIColor colorWithWhite:123.0/255.0 alpha:0.5];
...
{
    _minBound = [self minVerticalBound];
    _maxBound = [self maxVerticalBound];
    ...
    NSString* text = _labelForValue(_minBound + (_maxBound - _minBound) / _verticalGridStep * (index + 1));
    ...
    UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(3 - xOffset , p.y -5, width + 2, 14)];
    label.text = text;
...
- (UILabel*)createLabelForIndex: (NSUInteger)index
{
    ...
    //edited: krishna
    label.center = CGPointMake(p.x, label.center.y);
...
- (void)drawGrid
{
    ...
    if(_drawInnerGrid) {
        for(int i = 0;i <_horizontalGridStep;i++) {
            ...
            CGContextAddLineToPoint(ctx, point.x - 0.5f, _axisHeight + _margin);
            CGContextStrokePath(ctx);
        }
        
        for(int i=0;i<_verticalGridStep + 1;i++) {
            // If the value is zero then we display the horizontal axis
            CGFloat v = _maxBound - (_maxBound - _minBound) / _verticalGridStep * i;
...
- (void)strokeChart
{
    ...
    CGFloat boundLimit = (_maxBound - _minBound);
    boundLimit = (boundLimit != 0) ? boundLimit : 1;
    CGFloat scale = _axisHeight / boundLimit;
    ...
    if(_fillColorOne) {
        ...
        //EDITTED -  krishna
//        fillLayer.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + _minBound * scale-5, self.bounds.size.width, self.bounds.size.height);
        fillLayer.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + _minBound * scale, self.bounds.size.width, self.bounds.size.height);
        fillLayer.bounds = self.bounds;
        fillLayer.path = fill.CGPath;
        fillLayer.strokeColor = nil;
        fillLayer.fillColor = _fillColorOne.CGColor;
    ...
    //edited: krishna
//    pathLayer.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + _minBound * scale - 5, self.bounds.size.width, self.bounds.size.height);
    pathLayer.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + _minBound * scale, self.bounds.size.width, self.bounds.size.height);
...
//        fillLayer.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + _minBound * scale - 5, self.bounds.size.width, self.bounds.size.height);
        ...
        fillLayer.path = fill.CGPath;
        fillLayer.strokeColor = nil;
        fillLayer.fillColor = _fillColorTwo.CGColor;
        fillLayer.lineWidth = 0;
        fillLayer.lineJoin = kCALineJoinRound;
...
- (void)strokeDataPoints
{
    ...
    for(int i=0;i<_data.count;i++) {
        ...
        fillAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        fillAnimation.fillMode = kCAFillModeForwards;
        fillAnimation.fromValue = [NSNumber numberWithDouble:0.0];
        fillAnimation.toValue = (id)fillLayer.path;
        [fillLayer addAnimation:fillAnimation forKey:@"path"];
    }
}
